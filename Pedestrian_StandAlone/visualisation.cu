...
GLuint pedestrian_default_tbo;
GLuint pedestrian_default_displacementTex;
...
__global__ void output_pedestrian_agent_to_VBO(xmachine_memory_pedestrian_list* agents, glm::vec4* vbo, glm::vec3 centralise){
...
void initVisualisation()
{
	...
	// create TBO
	createTBO( &pedestrian_default_tbo, &pedestrian_default_displacementTex, xmachine_memory_pedestrian_MAX * sizeof( glm::vec4));
...
void runCuda()
{
	...
	if (get_agent_pedestrian_default_count() > 0)
	{
		...
		gpuErrchk(cudaGLMapBufferObject( (void**)&dptr, pedestrian_default_tbo));
		//cuda block size
		...
		output_pedestrian_agent_to_VBO<<< grid, threads>>>(get_device_pedestrian_default_agents(), dptr, centralise);
		gpuErrchkLaunch();
		// unmap buffer object
		gpuErrchk(cudaGLUnmapBufferObject(pedestrian_default_tbo));
	}
...
void display()
{
	...
	//Draw pedestrian Agents in default state
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_BUFFER_EXT, pedestrian_default_displacementTex);
	//loop
	for (int i=0; i< get_agent_pedestrian_default_count(); i++){
  ...
  frame_time += millis;
	...
	if(frame_count == average){
		char title [100];
		sprintf(title, "Execution & Rendering Total: %f (FPS), %f milliseconds per frame", average/(frame_time/1000.0f), frame_time/average);
...
void keyboard( unsigned char key, int /*x*/, int /*y*/)
{
	...
	case( 27) :
		...
		deleteTBO( &pedestrian_default_tbo);
